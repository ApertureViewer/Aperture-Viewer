...
void errorCallback(LLError::ELevel level, const std::string &error_string)
{
    if (level == LLError::LEVEL_ERROR)
    {
        ...
        {
            error_display_string = "We are sorry, but Aperture has crashed and needs to be closed. If you see this issue happening repeatedly, please contact our support team and submit the following message:\n\n[ERROR_DETAILS]";
...
bool LLAppViewer::initCache()
{
    ...
    // <FS:WW> Increase MAX_CACHE_SIZE to 1TB and comment out original FS:Beq 100GB limit
    const uintmax_t MIN_CACHE_SIZE = 256 * MB; // Keep MIN_CACHE_SIZE as is!
    ...
    constexpr U32 GB = 1024 * MB; // <FS:WW/> Readability constant
    const uintmax_t MAX_CACHE_SIZE = 1024ULL * GB; // <FS:WW/> Raise the cap to 1TB, removing the previous 100GB limit. UI still limits to a sensible level.
    // </FS:WW>
